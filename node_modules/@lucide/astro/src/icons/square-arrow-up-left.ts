
import createLucideIcon from '../createLucideIcon';
import type { AstroComponent, LucideIconData } from '../types'

const iconData: LucideIconData = {"name":"square-arrow-up-left","size":24,"node":[["path",{"d":"M15 15 9 9"}],["path",{"d":"M9 15V9h6"}],["rect",{"x":"3","y":"3","width":"18","height":"18","rx":"2"}]],"aliases":["arrow-up-left-square"]};

/**
 * @component @name SquareArrowUpLeft
 * @description Lucide SVG icon component, renders SVG Element with children.
 *
 * @preview ![img](data:image/svg+xml;base64,) - https://lucide.dev/icons/square-arrow-up-left
 * @see https://lucide.dev/guide/packages/lucide-astro - Documentation
 *
 * @param {import('../types').IconProps} props - Lucide icons props and any valid SVG attribute
 * @returns {any} Astro Component
 * 
 */
const SquareArrowUpLeft = createLucideIcon(iconData) as AstroComponent;

export default SquareArrowUpLeft;
