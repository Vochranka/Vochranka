
import createLucideIcon from '../createLucideIcon';
import type { AstroComponent, LucideIconData } from '../types'

const iconData: LucideIconData = {"name":"face-slightly-smiling","size":24,"node":[["path",{"d":"M15 10V9"}],["path",{"d":"M16.472 15a6 6 0 01-8.943 0"}],["path",{"d":"M9 10V9"}],["circle",{"cx":"12","cy":"12","r":"10"}]],"aliases":["smile"]};

/**
 * @component @name FaceSlightlySmiling
 * @description Lucide SVG icon component, renders SVG Element with children.
 *
 * @preview ![img](data:image/svg+xml;base64,) - https://lucide.dev/icons/face-slightly-smiling
 * @see https://lucide.dev/guide/packages/lucide-astro - Documentation
 *
 * @param {import('../types').IconProps} props - Lucide icons props and any valid SVG attribute
 * @returns {any} Astro Component
 * 
 */
const FaceSlightlySmiling = createLucideIcon(iconData) as AstroComponent;

export default FaceSlightlySmiling;
