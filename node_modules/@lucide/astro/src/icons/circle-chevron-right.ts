
import createLucideIcon from '../createLucideIcon';
import type { AstroComponent, LucideIconData } from '../types'

const iconData: LucideIconData = {"name":"circle-chevron-right","size":24,"node":[["circle",{"cx":"12","cy":"12","r":"10"}],["path",{"d":"m10 8 4 4-4 4"}]],"aliases":["chevron-right-circle"]};

/**
 * @component @name CircleChevronRight
 * @description Lucide SVG icon component, renders SVG Element with children.
 *
 * @preview ![img](data:image/svg+xml;base64,) - https://lucide.dev/icons/circle-chevron-right
 * @see https://lucide.dev/guide/packages/lucide-astro - Documentation
 *
 * @param {import('../types').IconProps} props - Lucide icons props and any valid SVG attribute
 * @returns {any} Astro Component
 * 
 */
const CircleChevronRight = createLucideIcon(iconData) as AstroComponent;

export default CircleChevronRight;
