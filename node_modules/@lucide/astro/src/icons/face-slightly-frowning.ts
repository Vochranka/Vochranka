
import createLucideIcon from '../createLucideIcon';
import type { AstroComponent, LucideIconData } from '../types'

const iconData: LucideIconData = {"name":"face-slightly-frowning","size":24,"node":[["path",{"d":"M15 10V9"}],["path",{"d":"M9 10V9"}],["path",{"d":"M9 16a5 5 0 016 0"}],["circle",{"cx":"12","cy":"12","r":"10"}]],"aliases":["frown"]};

/**
 * @component @name FaceSlightlyFrowning
 * @description Lucide SVG icon component, renders SVG Element with children.
 *
 * @preview ![img](data:image/svg+xml;base64,) - https://lucide.dev/icons/face-slightly-frowning
 * @see https://lucide.dev/guide/packages/lucide-astro - Documentation
 *
 * @param {import('../types').IconProps} props - Lucide icons props and any valid SVG attribute
 * @returns {any} Astro Component
 * 
 */
const FaceSlightlyFrowning = createLucideIcon(iconData) as AstroComponent;

export default FaceSlightlyFrowning;
