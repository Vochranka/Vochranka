
import createLucideIcon from '../createLucideIcon';
import type { AstroComponent, LucideIconData } from '../types'

const iconData: LucideIconData = {"name":"square-arrow-up-right","size":24,"node":[["path",{"d":"M15 15V9H9"}],["path",{"d":"m9 15 6-6"}],["rect",{"x":"3","y":"3","width":"18","height":"18","rx":"2"}]],"aliases":["arrow-up-right-square"]};

/**
 * @component @name SquareArrowUpRight
 * @description Lucide SVG icon component, renders SVG Element with children.
 *
 * @preview ![img](data:image/svg+xml;base64,) - https://lucide.dev/icons/square-arrow-up-right
 * @see https://lucide.dev/guide/packages/lucide-astro - Documentation
 *
 * @param {import('../types').IconProps} props - Lucide icons props and any valid SVG attribute
 * @returns {any} Astro Component
 * 
 */
const SquareArrowUpRight = createLucideIcon(iconData) as AstroComponent;

export default SquareArrowUpRight;
