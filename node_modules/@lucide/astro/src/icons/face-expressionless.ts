
import createLucideIcon from '../createLucideIcon';
import type { AstroComponent, LucideIconData } from '../types'

const iconData: LucideIconData = {"name":"face-expressionless","size":24,"node":[["path",{"d":"M14 10h2"}],["path",{"d":"M8 10h2"}],["path",{"d":"M8 16h8"}],["circle",{"cx":"12","cy":"12","r":"10"}]],"aliases":["annoyed"]};

/**
 * @component @name FaceExpressionless
 * @description Lucide SVG icon component, renders SVG Element with children.
 *
 * @preview ![img](data:image/svg+xml;base64,) - https://lucide.dev/icons/face-expressionless
 * @see https://lucide.dev/guide/packages/lucide-astro - Documentation
 *
 * @param {import('../types').IconProps} props - Lucide icons props and any valid SVG attribute
 * @returns {any} Astro Component
 * 
 */
const FaceExpressionless = createLucideIcon(iconData) as AstroComponent;

export default FaceExpressionless;
