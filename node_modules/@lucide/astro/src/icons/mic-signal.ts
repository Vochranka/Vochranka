
import createLucideIcon from '../createLucideIcon';
import type { AstroComponent, LucideIconData } from '../types'

const iconData: LucideIconData = {"name":"mic-signal","size":24,"node":[["path",{"d":"M12 17v4"}],["path",{"d":"M18 11a6 6 0 00-3-5.197"}],["path",{"d":"M2 11a10 10 0 015-8.662"}],["path",{"d":"M22 11a10 10 0 00-5-8.662"}],["path",{"d":"M6 11a6 6 0 013-5.197"}],["path",{"d":"M9 21h6"}],["rect",{"x":"10","y":"9","width":"4","height":"8","rx":"2"}]],"aliases":["podcast"]};

/**
 * @component @name MicSignal
 * @description Lucide SVG icon component, renders SVG Element with children.
 *
 * @preview ![img](data:image/svg+xml;base64,) - https://lucide.dev/icons/mic-signal
 * @see https://lucide.dev/guide/packages/lucide-astro - Documentation
 *
 * @param {import('../types').IconProps} props - Lucide icons props and any valid SVG attribute
 * @returns {any} Astro Component
 * 
 */
const MicSignal = createLucideIcon(iconData) as AstroComponent;

export default MicSignal;
